...
///
/// Both closures are `FnOnce + Send + 'static` so they can be moved into async
/// tasks without lifetime issues.
pub fn start<W, C>(cfg: TimerConfig, on_warning: W, on_cutoff: C)
where
