...
- **No third-party trackers** in the viewer. No analytics SDKs, no Google Fonts CDN, no external CSS. Self-host everything. *Exception:* a self-hosted Matomo on the same VPS (`musikersuche.org/matomo/`, Site ID 6) — **gated by Consent-Banner** (`viewer/public/matomo-consent.{js,css}` since DSGVO-review 2026-05-21). Pure vanilla JS, loaded externally from the four static marketing HTMLs (`viewer/index.html`, `impressum/`, `datenschutz/`, `download/`); only loads `matomo.js` after the user clicks „Statistik OK", and respects `navigator.doNotTrack` without even showing the banner. Decision-logic (truth table: consent × DNT) is tested in `viewer/tests/matomo-consent-decision.test.ts` against the pure helper at `viewer/src/matomo-consent-decision.ts`; the vanilla file duplicates those 4 lines so it can run outside Vite-bundling on the static legal pages. Matomo stays cookieless + DNT-respecting once loaded, no `enableLinkTracking`. **Inline `<script>` Matomo was the prior pattern (gh #96 → fecd506-era external → back-to-inline) — replaced 2026-05-21 because cookieless alone doesn't satisfy the strict §25-TTDSG reading by LfDI Niedersachsen; opt-in needed.** Matomo's own „tracking-code-validator" UI in the admin can no longer find the snippet via HEAD-curl (it expects inline), but the actual Tracker-URL + Site-ID are still in the JS so events POST correctly. The backend additionally fires one server-side `e_c=session&e_a=code_created` event per minted code via `backend/src/tracking/matomo.ts` (ENV-gated by `MATOMO_TRACKER_URL` + `MATOMO_SITE_ID`; absent env = silent no-op), AND inserts a per-mint row into the `code_events` DB-table (`backend/src/tracking/code_events.ts`, retention 365 d via purge.ts) as the verlässliche local Single-Source-of-Truth. No `cip`/`uid`/`url`/`urlref` is ever sent server-side. Disclosure: `viewer/public/datenschutz/index.html` §9 (opt-in-banner + cookieless mention) + §9a (Download-Counter, jetzt server-side via Stream-Proxy). The CSP-hashes in `caddy/Caddyfile` are jetzt nur noch zwei (die zwei JSON-LD-Inline-Blöcke); der dritte (Inline-Matomo) ist mit dem Consent-Banner-Refactor weg.
...
1. All tests pass: `npm test`, `cargo test`, etc. (Baseline at 2026-05-17: backend 365, sharer-lib 178 (+ 6 `#[ignore]` Display-requiring), viewer 176, dashboard 90. Drops are regressions. Run sharer's display-requiring tests via `cd sharer/src-tauri && cargo test --lib -- --ignored` on a host with X11/Wayland.)
