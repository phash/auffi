...
1. All tests pass: `npm test`, `cargo test`, etc. (Baseline at 2026-08-28 (v0.6.6): backend 470, sharer-lib 246 (+ 9 `#[ignore]` Display-requiring), viewer 583, dashboard 173, sharer-js 77. `purge.test.ts` hält timing-sensitive Scheduler-Tests, die in Full-Suite-Läufen intermittierend failen können — re-run the flaked file ISOLATED before believing a red run; keine Regression. Drops are regressions. Run sharer's display-requiring tests via `cd sharer/src-tauri && cargo test --lib -- --ignored` on a host with X11/Wayland.)
