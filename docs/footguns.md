...
- **RTP timestamps MUST track the frame's capture time** (`sharer/src-tauri/src/rtp_clock.rs`, consumed by `streaming_loop`). The sharer packetizes onto a `TrackLocalStaticRTP` itself; `TrackLocalStaticSample` with a constant `Sample.duration` was the pre-0.7.1 shape and claimed 33 ms between frames that WGC/pipewiresrc delivered 300–500 ms apart on a static screen. No transport-cc/abs-send-time extension is stamped on outgoing RTP, so Chrome falls back to its receive-side estimator, which compares arrival gaps with RTP-timestamp gaps: the systematic mismatch reads as overuse, the REMB collapses to a few kbps, and since REMB is a hard cap in `bitrate_controller.rs` the encoder sat at the 150 kbps floor while idle ("blurry after idle"). Anyone reintroducing a fixed per-frame duration recreates that. The clock steps a nominal 33 ms when pts goes backwards (fresh capturer after `switch_monitor`) and caps a single gap at 2 s.
...
- **Four env-driven per-IP caps**: `rate_limit` (`RATE_LIMIT_MAX`, default 5 — viewer `join`s on `/signal`), `register_rate_limit`, `bearer_auth_rate_limit` (signaling `/signal` upgrade, Sec H-1), and `feedback_bearer_rate_limit` (`FEEDBACK_BEARER_RATE_LIMIT_MAX`, default 5 — gates argon2 on `POST /api/feedback`'s sharer-Bearer branch). All four share one fixed-window limiter, `checkIpRateLimit` in `backend/src/rate-limit.ts`; their bucket-Maps are swept every 60 s in `server.ts`. **The join budget is consumed on EVERY well-formed attempt, hits included, and is checked BEFORE the code lookup** — otherwise an IP that burned its budget on wrong guesses would still be attached on its first correct one and the documented 5/min guessing bound would be cosmetic. Consequence for tests (and a local Playwright backend): anything that joins more than 5× per minute from `127.0.0.1` must set `RATE_LIMIT_MAX=1000` alongside `REGISTER_RATE_LIMIT_MAX=1000` / `BEARER_AUTH_RATE_LIMIT_MAX=1000`.
